...
{
    public function test_priority_and_inquiry_status_are_master_color_types(): void
    {
        $this->assertContains('priority', MasterDataService::COLOR_TYPES);
        $this->assertContains('inquiry_status', MasterDataService::COLOR_TYPES);
        $this->assertSame('#DC2626', MasterColor::defaultFor('priority', 'Critical'));
        $this->assertSame('#D97706', MasterColor::defaultFor('inquiry_status', 'Waiting for Client'));
    }
    ...
    public function test_master_data_ui_and_main_surfaces_use_master_colors(): void
    {
        ...
        $this->assertStringContainsString("displayColorFor('inquiry_status'", $inquiryView);
        $this->assertStringContainsString("displayColorFor('priority'", $inquiryView);
        $this->assertStringContainsString("displayColorFor('inquiry_status'", $dashboardView);
