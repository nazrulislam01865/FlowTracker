...
@forelse($phaseCounts->sortBy(fn($row) => $row->phase?->sequence ?? PHP_INT_MAX)->take(6) as $row)<div class="phase-row"><span>{{ $row->phase?->short_name ?? 'Unassigned' }}</span><x-ui.progress :value="min(100,max(8,$row->total*16))" /><b>{{ $row->total }}</b></div>@empty<div class="empty-state">No active Jobs by phase.</div>@endforelse
</div></div>
</div>
<div class="grid-3">
<div class="card section-card"><div class="section-head"><h3>Team Workload</h3><a class="link-btn" href="{{ route('reports') }}" wire:navigate>Details</a></div>@forelse($workload as $person)<div class="workload-row"><div class="person"><x-ui.avatar :name="$person->name" :size="27" />{{ $person->name }}</div><x-ui.progress :value="min(100,$person->open_tasks_count*12)"/><b>{{ $person->open_tasks_count }}</b></div>@empty<div class="empty-state">No active team workload.</div>@endforelse</div>
<div class="card section-card"><div class="section-head"><h3>Upcoming Deliveries</h3><a class="link-btn" href="{{ route('jobs.index') }}" wire:navigate>View Jobs</a></div><table class="mini-table"><thead><tr><th>Job</th><th>Client</th><th>Delivery</th></tr></thead><tbody>@forelse($deliveries as $job)<tr><td><a class="job-link" href="{{ route('jobs.index',['open'=>$job->id]) }}" wire:navigate>{{ str($job->job_number)->afterLast('-') }}</a><div class="muted small truncate" style="max-width:125px">{{ $job->title }}</div></td><td>{{ str($job->client?->name ?? '—')->before(' ') }}</td><td>{{ $job->delivery_date?->format('M j, Y') }}</td></tr>@empty<tr><td colspan="3"><div class="empty-state">No upcoming deliveries.</div></td></tr>@endforelse</tbody></table></div>
