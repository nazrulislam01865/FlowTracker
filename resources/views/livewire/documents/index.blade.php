<div class="ft-documents-page">
    ...
    @if($showUpload)
        <div class="overlay livewire-overlay" wire:click.self="closeUpload"></div><div class="modal livewire-modal ft-doc-upload-modal"><div class="modal-head"><div><h2>Upload document</h2><div class="small muted">Link the file to a visible Job and optionally to one of your assigned tasks.</div></div><button class="close-btn" wire:click="closeUpload">×</button></div><div class="modal-body"><div class="form-grid"><div class="field"><label>Job *</label><select wire:model.live="uploadJobId"><option value="">Select Job</option>@foreach($jobs as $j)<option value="{{ $j->id }}">{{ $j->job_number }} · {{ $j->title }}</option>@endforeach</select></div><div class="field"><label>Task</label><select wire:model="uploadTaskId"><option value="">Job-level document</option>@foreach($uploadTasks as $task)<option value="{{ $task->id }}">{{ $task->phase?->short_name }} · {{ $task->title }}</option>@endforeach</select></div><div class="field full"><label>Document type *</label><select wire:model="uploadCategory">@foreach($categories as $cat)<option>{{ $cat->name }}</option>@endforeach</select></div><div class="field full" x-data="{ uploading:false, progress:0 }" x-on:livewire-upload-start="uploading=true; progress=0" x-on:livewire-upload-progress="progress=$event.detail.progress" x-on:livewire-upload-error="uploading=false; progress=0" x-on:livewire-upload-finish="progress=100; setTimeout(() => { uploading=false; progress=0 }, 700)"><label class="upload-zone"><input type="file" wire:model="files" multiple><b>Drop files here or browse</b><div class="small muted">PDF, DOCX, XLSX, JPG, PNG, ZIP, TXT or CSV · Max 20 MB each</div></label><div class="ft-upload-progress-wrap" x-cloak x-show="uploading" x-transition.opacity><div class="ft-upload-progress-copy"><span>Uploading document…</span><b x-text="progress + '%'">0%</b></div><div class="ft-upload-progress-track"><span x-bind:style="`width:${progress}%`"></span></div></div></div></div>@error('files.*')<div class="validation-error">{{ $message }}</div>@enderror</div><div class="modal-foot"><button class="ghost" wire:click="closeUpload">Cancel</button><button class="primary" wire:click="upload" wire:loading.attr="disabled">Upload document</button></div></div>
    @endif
</div>
