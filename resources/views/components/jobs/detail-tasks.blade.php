        ...
        @foreach($job->workflow->phases as $phase)
            ...
            <div class="ft-phase-group open">
                ...
                @forelse($phaseTasks as $task)
                    <button class="ft-phase-task-line" wire:click="openTask({{ $task->id }})"><span>{{ $phase->sequence }}.{{ $loop->iteration }}</span><b>{{ $task->title }}</b><span><x-ui.avatar :name="$task->assignee?->name ?? 'Unassigned'" :size="24"/>{{ $task->assignee?->name ?? 'Unassigned' }}</span><span>{{ $task->due_date?->format('M j, Y') ?? '—' }}</span><span class="ft-soft-pill {{ \App\Support\JobDetailPresenter::taskStatusClass($task->status) }}">{{ $task->status }}</span><span>•••</span></button>
                @empty
