...
<article {{ $attributes->class(['ft-task-board-card']) }}>
    <div class="ft-task-board-top">
        <div class="ft-task-card-badges"><span class="ft-priority-pill {{ strtolower($task->priority) }}">{{ $task->priority }}</span>@if($task->needs_attention)<span class="ft-attention-pill">⚑ Attention needed</span>@endif</div>
