...
<div <?php echo e($attributes->class(['ft-jl-control', 'ft-remote-filter', 'ft-local-filter', 'is-disabled' => $disabled])); ?>
    ...
    x-effect="sync(<?php echo \Illuminate\Support\Js::from((string)$value)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($resolvedLabel)->toHtml() ?>)"
    ...
    <div x-ref="menu" class="ft-remote-filter-menu ft-local-filter-menu" x-cloak x-show="open"
        ...
        <input x-ref="search" class="ft-remote-filter-search" type="text" role="searchbox" inputmode="search"
            ...
            placeholder="Search <?php echo e(strtolower($label)); ?>…" autocomplete="off">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($clearable): ?>
            <button type="button" class="ft-remote-filter-option ft-remote-filter-clear" :aria-selected="selectedValue === ''" x-on:click="choose('', <?php echo \Illuminate\Support\Js::from($placeholder)->toHtml() ?>); $wire.set(<?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, '')">
        ...
        <div class="ft-remote-filter-list" role="listbox">
            ...
            <template x-for="item in filteredItems" :key="item.id">
                <button type="button" class="ft-remote-filter-option" :aria-selected="String(item.id) === String(selectedValue)" x-on:click="choose(String(item.id), item.label); $wire.set(<?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, String(item.id))">
