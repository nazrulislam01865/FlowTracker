...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.ui.search-select','data' => ['class' => 'assignee-filter','label' => 'Assignee','property' => 'assignee','value' => $assignee,'placeholder' => 'All','options' => collect([['id' => 'unassigned', 'label' => 'Unassigned']])->concat(
                        $assigneeFilterOptions->map(fn ($option) => [
                            'id' => (string) $option->id,
                            'label' => $option->name,
                        ])
                    ),'searchPlaceholder' => 'Search people…','hideLabel' => true,'fixedMenu' => true,'menuWidth' => 300,'wire:key' => 'all-tasks-assignee-filter-'.e($assignee ?: 'all').'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['class' => 'assignee-filter','label' => 'Assignee','property' => 'assignee','value' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($assignee),'placeholder' => 'All','options' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(collect([['id' => 'unassigned', 'label' => 'Unassigned']])->concat(
                        $assigneeFilterOptions->map(fn ($option) => [
                            'id' => (string) $option->id,
                            'label' => $option->name,
                        ])
                    )),'search-placeholder' => 'Search people…','hide-label' => true,'fixed-menu' => true,'menu-width' => 300,'wire:key' => 'all-tasks-assignee-filter-'.e($assignee ?: 'all').'']); ?>
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
