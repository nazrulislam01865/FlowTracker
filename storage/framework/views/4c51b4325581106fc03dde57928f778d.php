            <section id="job-document-upload-panel" class="ft-detail-card ft-upload-panel ft-doc-upload-card ft-prototype-document-uploader">
                ...
                <?php if ($required->isNotEmpty()): ?>
                    <div class="ft-proto-doc-field">
                        <label for="jobDocumentRequirement-<?php echo e($job->id); ?>">Document type</label>
                        <select id="jobDocumentRequirement-<?php echo e($job->id); ?>" wire:model.live="jobDocumentTaskId" class="ft-proto-doc-select">
                        ...
                        <?php if ($errors->has('jobDocumentTaskId')): ?><div class="validation-error ft-field-validation" role="alert"><?php echo e($errors->first('jobDocumentTaskId')); ?></div><?php endif; ?>
                    </div>
                    ...
                    <div class="ft-proto-doc-mode" role="tablist" aria-label="Document source">
                        <?php if ($canUploadDocument): ?>
                            <button type="button" role="tab" aria-selected="<?php echo e(! $showDocumentPicker ? 'true' : 'false'); ?>" class="<?php echo e(! $showDocumentPicker ? 'is-active' : ''); ?>" wire:click="setDocumentUploadMode('upload')">
                        ...
                        <?php if ($canLinkDocument): ?>
                            <button type="button" role="tab" aria-selected="<?php echo e(($showDocumentPicker || ! $canUploadDocument) ? 'true' : 'false'); ?>" class="<?php echo e(($showDocumentPicker || ! $canUploadDocument) ? 'is-active' : ''); ?>" wire:click="setDocumentUploadMode('existing')">
                    ...
                    <?php if ($canUploadDocument && ! $showDocumentPicker): ?>
                        <div
                            ...
                            <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = 'job-required-upload-'.e($job->id).'-'.e($uploadInitialState).'-'.e((int) ($lastUploadedDocument?->id ?? 0)).'-'.e(md5((string) $uploadError)).''; ?>wire:key="job-required-upload-<?php echo e($job->id); ?>-<?php echo e($uploadInitialState); ?>-<?php echo e((int) ($lastUploadedDocument?->id ?? 0)); ?>-<?php echo e(md5((string) $uploadError)); ?>"
                            data-file-dropzone
                            x-data="{
                                state: <?php echo \Illuminate\Support\Js::from($uploadInitialState)->toHtml() ?>,
                                progress: 0,
                                fileName: <?php echo \Illuminate\Support\Js::from($lastUploadedDocument?->name ?: $pendingUploadName)->toHtml() ?>,
                                fileSize: <?php echo \Illuminate\Support\Js::from($lastUploadedDocument?->size ?: $pendingUploadSize)->toHtml() ?>,
                                errorText: <?php echo \Illuminate\Support\Js::from($uploadError ?: 'The connection was interrupted. Your document was not added.')->toHtml() ?>,
                                selectedFile: null,
                                uploadToken: 0,
                                prettySize(bytes) {
                                ...
                                validateFile(file) {
                                    ...
                                    const allowed = <?php echo \Illuminate\Support\Js::from(\App\Support\AttachmentUpload::extensions())->toHtml() ?>;
                                    if (!allowed.includes(extension)) {
                                        this.errorText = <?php echo \Illuminate\Support\Js::from(\App\Support\AttachmentUpload::validationMessage())->toHtml() ?>;
                                        return false;
                                    }
                        ...
                        >
                            <input
                                ...
                                id="jobDocumentUpload-<?php echo e($job->id); ?>"
                                type="file"
                            ...
                            <?php if ($lastUploadedDocument): ?>
                                <div class="ft-proto-upload-state ft-proto-upload-success" x-show="state === 'success'" x-cloak>
                                    <div class="ft-proto-file-summary">
                                        <span class="ft-proto-file-icon"><?php echo e(strtoupper(pathinfo($lastUploadedDocument->name, PATHINFO_EXTENSION) ?: 'FILE')); ?></span>
                                        <div class="ft-proto-file-copy">
                                            <strong><?php echo e($lastUploadedDocument->name); ?></strong>
                                            <span><?php echo e(number_format(((int) $lastUploadedDocument->size) / 1048576, 1)); ?> MB · Uploaded just now</span>
                                    ...
                                    <div class="ft-proto-success-actions">
                                        <a class="ft-proto-outline-action" href="<?php echo e(route('documents.open', $lastUploadedDocument)); ?>" target="_blank" rel="noopener">View document</a>
                        ...
                        <div class="ft-proto-upload-formats"><?php echo e(\App\Support\AttachmentUpload::helperText(20)); ?></div>
                    <?php elseif ($canLinkDocument && ($showDocumentPicker || ! $canUploadDocument)): ?>
                        <div class="ft-proto-existing-panel">
                            ...
                            <?php if ($availableDocuments->isNotEmpty()): ?>
                                ...
                                <?php if ($errors->has('existingDocumentId')): ?><div class="validation-error ft-field-validation"><?php echo e($errors->first('existingDocumentId')); ?></div><?php endif; ?>
                            <?php else: ?>
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/laravel/StepPromo/resources/views/components/jobs/documents/required-document-uploader.blade.php ENDPATH**/ ?>