    ...
    >
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($clearable): ?>
            <button
                ...
                x-on:click.stop="<?php if($remote): ?> clearSelection() <?php else: ?> choose('', <?php echo \Illuminate\Support\Js::from($placeholder)->toHtml() ?>) <?php endif; ?>; $dispatch('flowtrack-selection-changed', {property: <?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, value: '', label: <?php echo \Illuminate\Support\Js::from($placeholder)->toHtml() ?>}); $nextTick(() => Promise.resolve(<?php if($action): ?> $wire.call(<?php echo \Illuminate\Support\Js::from($action)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, '') <?php else: ?> $wire.set(<?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, '') <?php endif; ?>).catch(() => <?php if($remote): ?> selectionFailed() <?php else: ?> null <?php endif; ?>))"
            >
