...
<?php endif; ?><b><?php echo e($task->assignee?->name ?? 'Unassigned'); ?></b></div>
        <span
            ...
            x-data="window.FlowTrackInlineEdit({ key: <?php echo \Illuminate\Support\Js::from('task-'.$task->id.'-due-date')->toHtml() ?>, label: 'task due date', value: <?php echo \Illuminate\Support\Js::from($task->due_date?->format('Y-m-d') ?? '')->toHtml() ?>, display: <?php echo \Illuminate\Support\Js::from($task->due_date?->format('M j') ?? 'Set due date')->toHtml() ?> })"
