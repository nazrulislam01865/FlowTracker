...
})" x-effect="sync(<?php echo \Illuminate\Support\Js::from((string)$value)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($resolvedLabel)->toHtml() ?>)" x-on:keydown.escape.window="close()" x-on:resize.window="open && reposition()" x-on:scroll.window="open && reposition()">
    ...
    <div x-ref="menu" class="ft-remote-filter-menu" x-cloak x-show="open" x-bind:style="menuStyle" x-on:click.outside="close()" x-on:keydown.arrow-down.prevent="moveOption(1)" x-on:keydown.arrow-up.prevent="moveOption(-1)">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($clearable): ?>
            <button type="button" class="ft-remote-filter-option ft-remote-filter-clear" x-show="selectedValue"
                x-on:click="clearSelection(); <?php if($action): ?> $wire.call(<?php echo \Illuminate\Support\Js::from($action)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, '') <?php else: ?> $wire.set(<?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, '') <?php endif; ?>"><span><?php echo e($placeholder); ?></span><small>Clear</small></button>
        ...
        <div class="ft-remote-filter-list" role="listbox">
            ...
            <template x-for="item in items" :key="item.id">
                <button type="button" class="ft-remote-filter-option" :aria-selected="String(item.id) === String(selectedValue)"
                    x-on:click="select(item); <?php if($action): ?> $wire.call(<?php echo \Illuminate\Support\Js::from($action)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, String(item.id)) <?php else: ?> $wire.set(<?php echo \Illuminate\Support\Js::from($property)->toHtml() ?>, String(item.id)) <?php endif; ?>">
