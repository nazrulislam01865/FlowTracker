...
>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($ownerTask): ?>
        <div
            ...
            x-on:task-assignee-updated.window="if (Number($event.detail?.taskId) === Number(<?php echo e((int) $ownerTask->id); ?>)) syncConfirmed(String($event.detail?.assigneeId ?? ''), String($event.detail?.assigneeName ?? 'Unassigned'), { avatarUrl:String($event.detail?.avatarUrl ?? '') })"
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/laravel/StepPromo/resources/views/components/jobs/order-detail/stage-card.blade.php ENDPATH**/ ?>