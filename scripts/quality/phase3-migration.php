...
foreach (($baseline['ceilings'] ?? []) as $name => $limit) {
    if (isset($metrics[$name]) && $metrics[$name] > (int) $limit) {
        $failures[] = "$name increased to {$metrics[$name]} (ceiling $limit)";
    }
}
...
echo "\nPASS: Phase 3 CSS assets and their tests are source-managed, Blade style blocks are removed from application views, and migration debt did not increase.\n";
