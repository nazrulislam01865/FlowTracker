...
class FlowTrackDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $users = collect($people)->mapWithKeys(function ($p) use ($roles, $departments) {
            ...
            WorkspaceMembership::updateOrCreate(['workspace_id'=>1,'user_id'=>$u->id], ['role_id'=>$u->role_id,'department_id'=>$u->department_id,'status'=>'active','joined_at'=>$u->created_at ?: now()]);
            return [$p[0] => $u];
        });
