(() => {
    ...
    const browserNormalizeExcel = async (file) => {
        ...
        if (!['xlsx', 'xls'].includes(extension) || !window.XLSX) return { uploadFile: file, displayFilename: null, fingerprint: null };

        const buffer = await file.arrayBuffer();
        const workbook = window.XLSX.read(buffer, { type: 'array', cellDates: true, cellText: false });
        if (!workbook.SheetNames?.length) throw new Error('The spreadsheet does not contain a worksheet.');
        const firstSheet = workbook.Sheets[workbook.SheetNames[0]];
        const csv = window.XLSX.utils.sheet_to_csv(firstSheet, { blankrows: true, dateNF: 'yyyy-mm-dd' });
        return {
